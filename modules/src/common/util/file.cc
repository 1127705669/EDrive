...
std::string GetRosHome() {
  ...
  if (ros_root == nullptr || !EndWith(ros_root, kKnownTail)) {
    EERROR << "Failed to find ROS root: " << ros_root;
...
bool CopyFile(const std::string &from, const std::string &to) {
  ...
  if (!src) {
    EERROR << "Source path doesn't exist: " << from;
    return false;
  }

  std::ofstream dst(to, std::ios::binary);
  if (!dst) {
    EERROR << "Target path is not writable: " << to;
    return false;
  }
...
bool CopyDir(const std::string &from, const std::string &to) {
  ...
  if (directory == nullptr) {
    EERROR << "Cannot open directory " << from;
    return false;
  }
  ...
  if (EnsureDirectory(to)) {
    ...
    EERROR << "Cannot create target directory " << to;
    ret = false;
  }
  closedir(directory);
  return ret;
...
bool RemoveAllFiles(const std::string &directory_path) {
  ...
  if (directory == nullptr) {
    EERROR << "Cannot open directory " << directory_path;
    return false;
  }
  ...
  while ((file = readdir(directory)) != nullptr) {
    ...
    if (unlink(file_path.c_str()) < 0) {
      EERROR << "Fail to remove file " << file_path << ": " << strerror(errno);
      closedir(directory);
      return false;
    }
  }
