...
bool SetProtoToASCIIFile(const MessageType &message, int file_descriptor) {
  ...
  if (file_descriptor < 0) {
    EERROR << "Invalid file descriptor.";
    return false;
  }
...
bool SetProtoToASCIIFile(const MessageType &message,
  ...
  if (fd < 0) {
    EERROR << "Unable to open file " << file_name << " to write.";
    return false;
  }
...
bool GetProtoFromASCIIFile(const std::string &file_name, MessageType *message) {
  ...
  if (file_descriptor < 0) {
    EERROR << "Failed to open file " << file_name << " in text mode.";
    // Failed to open;
    return false;
  }
  ...
  if (!success) {
    EERROR << "Failed to parse file " << file_name << " as text proto.";
  }
  delete input;
  close(file_descriptor);
  return success;
...
bool GetProtoFromBinaryFile(const std::string &file_name,
  ...
  if (!input.good()) {
    EERROR << "Failed to open file " << file_name << " in binary mode.";
    return false;
  }
  if (!message->ParseFromIstream(&input)) {
    EERROR << "Failed to parse file " << file_name << " as binary proto.";
    return false;
  }
  return true;
}
