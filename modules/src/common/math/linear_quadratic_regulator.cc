...
void SolveLQRProblem(const Matrix &A, const Matrix &B, const Matrix &Q,
  ...
  if (A.rows() != A.cols() || B.rows() != A.rows() || Q.rows() != Q.cols() ||
    ...
    EERROR << "LQR solver: one or more matrices have incompatible dimensions.";
    return;
  }
  ...
  if (num_iteration >= max_num_iteration) {
    EDEBUG << "LQR solver cannot converge to a solution, "
              "last consecutive result diff is: "
           << diff;
  } else {
    EDEBUG << "LQR solver converged at iteration: " << num_iteration
