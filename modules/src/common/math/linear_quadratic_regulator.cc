...
void SolveLQRProblem(const Matrix &A, const Matrix &B, const Matrix &Q,
  ...
  if (num_iteration >= max_num_iteration) {
    ROS_WARN("LQR solver cannot converge to a solution, last consecutive result diff. is: %f",diff);
  } else {
    ROS_DEBUG("LQR solver converged at iteration: %d, max consecutive result diff.: %f", num_iteration, diff);
  }
