...
#include "routing/core/black_list_range_generator.h"
...
double MoveSForward(double s, double upper_bound) {
  if (s > upper_bound) {
    EERROR << "Illegal s: " << s << ", upper bound: " << upper_bound;
    return s;
  }
...
double MoveSBackward(double s, double lower_bound) {
  if (s < lower_bound) {
    EERROR << "Illegal s: " << s << ", lower bound: " << lower_bound;
    return s;
  }
...
void BlackListRangeGenerator::AddBlackMapFromTerminal(
  ...
  if (start_s < 0.0 || start_s > start_length) {
    EERROR << "Illegal start_s: " << start_s << ", length: " << start_length;
    return;
  }
  if (end_s < 0.0 || end_s > end_length) {
    EERROR << "Illegal end_s: " << end_s << ", length: " << end_length;
    return;
  }
