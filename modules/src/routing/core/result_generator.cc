/******************************************************************************
  ...
  *****************************************************************************/
#include "routing/core/result_generator.h"
...
#include "routing/graph/range_utils.h"
...
bool ResultGenerator::ExtractBasicPassages(
  ...
  for (size_t i = 1; i < nodes.size(); ++i) {
    ...
    if (edge == nullptr) {
      EERROR << "Get null pointer to edge from " << nodes.at(i - 1).LaneId()
...
void PrintDebugInfo(const std::string& road_id,
  ...
  EINFO << "road id: " << road_id;
  for (size_t j = 0; j < nodes.size(); ++j) {
    EINFO << "\tPassage " << j;
    for (const auto& node : nodes[j]) {
      EINFO << "\t\t" << node.LaneId() << "   (" << node.StartS() << ", "
            << node.EndS() << ")";
    }
  }
}
