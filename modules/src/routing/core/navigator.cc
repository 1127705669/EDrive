...
#include "routing/strategy/a_star_strategy.h"
...
bool ShowRequestInfo(const RoutingRequest& request, const TopoGraph* graph) {
  for (const auto& wp : request.waypoint()) {
    ...
    if (node == nullptr) {
      EERROR << "Way node is not found in topo graph! ID: " << wp.id();
      return false;
    }
    EINFO << "Way point:\tlane id: " << wp.id() << " s: " << wp.s()
  ...
  for (const auto& bl : request.blacklisted_lane()) {
    ...
    if (node == nullptr) {
      EERROR << "Black list node is not found in topo graph! ID: " << bl.id();
      return false;
    }
    EINFO << "Black point:\tlane id: " << bl.id()
...
bool GetWayNodes(const RoutingRequest& request, const TopoGraph* graph,
  ...
  for (const auto& point : request.waypoint()) {
    ...
    if (cur_node == nullptr) {
      EERROR << "Can't find way point in graph! Id: " << point.id();
      return false;
    }
...
void SetErrorCode(const common::ErrorCode& error_code_id,
  ...
  if (error_code_id == common::ErrorCode::OK) {
    EDEBUG << error_string.c_str();
  } else {
    EERROR << error_string.c_str();
  }
}

void PrintDebugData(const std::vector<NodeWithRange>& nodes) {
  EINFO << "Route lane id\tis virtual\tstart s\tend s";
  for (const auto& node : nodes) {
    EINFO << node.GetTopoNode()->LaneId() << "\t"
...
Navigator::Navigator(const std::string& topo_file_path) {
  ...
  if (!common::util::GetProtoFromFile(topo_file_path, &graph)) {
    EERROR << "Failed to read topology graph from " << topo_file_path;
    return;
  }

  graph_.reset(new TopoGraph());
  if (!graph_->LoadGraph(graph)) {
    EINFO << "Failed to init navigator graph failed! File path: "
          << topo_file_path;
    return;
  }
...
bool Navigator::Init(const RoutingRequest& request, const TopoGraph* graph,
  ...
  if (!GetWayNodes(request, graph_.get(), way_nodes, way_s)) {
    EERROR << "Failed to find search terminal point in graph!";
    return false;
  }
  ...
  for (const auto& node : node_vec) {
    if (result_node_vec->empty() ||
      ...
      if (result_node_vec->back().EndS() < node.StartS()) {
        EERROR << "Result route is not coninuous";
        return false;
      } else {
...
bool Navigator::SearchRouteByStrategy(
  ...
  for (size_t i = 1; i < way_nodes.size(); ++i) {
    ...
    if (start == nullptr) {
      EERROR << "Sub graph node is nullptr, origin node id: "
    ...
    if (!strategy_ptr->Search(graph, &sub_graph, start, end,
                              &cur_result_nodes)) {
      EERROR << "Failed to search route with waypoint from " << start->LaneId()
             << " to " << end->LaneId();
      return false;
    }
