...
bool GraphCreator::Create() {
  if (common::util::EndWith(base_map_file_path_, ".xml")) {
    if (!hdmap::adapter::OpendriveAdapter::LoadData(base_map_file_path_,
                                                    &pbmap_)) {
      EERROR << "Failed to load base map file from " << base_map_file_path_;
      return false;
    }
  } else {
  ...
  EINFO << "Number of lanes: " << pbmap_.lane_size();
  ...
  for (const auto& lane : pbmap_.lane()) {
    ...
    if (forbidden_lane_id_set_.find(lane_id) != forbidden_lane_id_set_.end()) {
      EDEBUG << "Ignored lane id: " << lane_id
             << " because its type is NOT CITY_DRIVING.";
      continue;
    }
    EINFO << "Current lane id: " << lane_id;
  ...
  if (!EndWith(dump_topo_file_path_, ".bin") &&
    ...
    EERROR << "Failed to dump topo data into file, incorrect file type "
  ...
  if (!common::util::SetProtoToASCIIFile(graph_, txt_file)) {
    EERROR << "Failed to dump topo data into file " << txt_file;
    return false;
  }
  EINFO << "Txt file is dumped successfully. Path: " << txt_file;
  if (!common::util::SetProtoToBinaryFile(graph_, bin_file)) {
    EERROR << "Failed to dump topo data into file " << bin_file;
    return false;
  }
  EINFO << "Bin file is dumped successfully. Path: " << bin_file;
  return true;
}
...
void GraphCreator::AddEdge(const Node& from_node,
  ...
  for (const auto& to_id : to_node_vec) {
    if (forbidden_lane_id_set_.find(to_id.id()) !=
      ...
      EDEBUG << "Ignored lane [id = " << to_id.id();
      continue;
    }
