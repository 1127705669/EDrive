...
Result_state LanesXmlParser::Parse(const tinyxml2::XMLElement& xml_node,
...
Result_state LanesXmlParser::ParseSectionBoundary(
...
Result_state LanesXmlParser::ToPbBoundaryType(const std::string& type,
                                        PbBoundaryEdgeType* boundary_type) {
...
Result_state LanesXmlParser::ParseLaneSection(const tinyxml2::XMLElement& xml_node,
...
Result_state LanesXmlParser::ParseLane(const tinyxml2::XMLElement& xml_node,
                                 LaneInternal* lane_internal) {
  ...
  Result_state success = ToPbLaneType(lane_type, &pb_lane_type);
  if (success != Result_state::State_Ok) {
...
Result_state LanesXmlParser::ParseDirection(const tinyxml2::XMLElement& xml_node,
                                      PbLane* lane) {
  CHECK_NOTNULL(lane);
  ...
  Result_state success = ToPbDirection(direction, &pb_lane_direction);
  if (success != Result_state::State_Ok) {
...
Result_state LanesXmlParser::ParseCenterCurve(const tinyxml2::XMLElement& xml_node,
                                        PbLane* lane) {
  CHECK_NOTNULL(lane);
  ...
  Result_state success = UtilXmlParser::ParseCurve(*sub_node, center_curve);
  if (success != Result_state::State_Ok) {
    std::string err_msg = "Error parse lane center curve";
    return Result_state::State_Failed;
  }
...
Result_state LanesXmlParser::ParseSpeed(const tinyxml2::XMLElement& xml_node,
                                  PbLane* lane) {
...
Result_state LanesXmlParser::ParseSampleAssociates(
...
Result_state LanesXmlParser::ParseRoadSampleAssociates(
    const tinyxml2::XMLElement& xml_node, PbLane* lane) {
  CHECK_NOTNULL(lane);
...
Result_state LanesXmlParser::ParseObjectOverlapGroup(
...
Result_state LanesXmlParser::ParseSignalOverlapGroup(
    const tinyxml2::XMLElement& xml_node,
...
Result_state LanesXmlParser::ParseJunctionOverlapGroup(
    const tinyxml2::XMLElement& xml_node,
...
Result_state LanesXmlParser::ParseLaneOverlapGroup(
    const tinyxml2::XMLElement& xml_node,
...
Result_state LanesXmlParser::ToPbLaneType(const std::string& type,
...
Result_state LanesXmlParser::ToPbTurnType(const std::string& type,
                                    PbTurnType* pb_turn_type) {
...
Result_state LanesXmlParser::ToPbDirection(const std::string& type,
...
Result_state LanesXmlParser::ParseLaneBorderMark(
    const tinyxml2::XMLElement& xml_node,
    PbLaneBoundaryTypeType* boundary_type) {
  CHECK_NOTNULL(boundary_type);
  ...
  Result_state success = ToPbLaneMarkType(type, color, boundary_type);
  if (success != Result_state::State_Ok) {
...
Result_state LanesXmlParser::ToPbLaneMarkType(const std::string& type,
