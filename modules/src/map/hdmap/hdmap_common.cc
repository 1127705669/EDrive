...
namespace hdmap {
namespace {
using EDrive::common::PointENU;
using EDrive::common::math::Vec2d;
...
void PointsFromCurve(const Curve &input_curve, std::vector<Vec2d> *points) {
  ...
  for (const auto &curve : input_curve.segment()) {
    if (curve.has_line_segment()) {
      ...
      EERROR << "Can not handle curve type.";
    }
  }
  RemoveDuplicates(points);
}

EDrive::common::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) {
  ...
  while (points.size() >= 2 && points[0].DistanceTo(points.back()) <=
                                   EDrive::common::math::kMathEpsilon) {
    points.pop_back();
  }
  return EDrive::common::math::Polygon2d(points);
}

void SegmentsFromCurve(
    const Curve &curve,
    std::vector<EDrive::common::math::LineSegment2d> *segments) {
...
void LaneInfo::Init() {
  ...
  if (lane_.has_type()) {
    if (lane_.type() == Lane::CITY_DRIVING) {
      ...
      EERROR << "lane_[id = " << lane_.id().DebugString() << " type is NONE.";
    }
  } else {
    EERROR << "lane_[id = " << lane_.id().DebugString() << "has NO type.";
  }
...
double LaneInfo::Heading(const double s) const {
  ...
  if (s + kEpsilon < accumulated_s_.front()) {
    EERROR << "s:" << s << " should be >= " << accumulated_s_.front();
    return 0.0;
  }
  if (s - kEpsilon > accumulated_s_.back()) {
    EERROR << "s:" << s << " should be <= " << accumulated_s_.back();
    return 0.0;
  }
...
double LaneInfo::Curvature(const double s) const {
  if (points_.size() < 2) {
    EERROR << "Not enough points to compute curvature.";
    return 0.0;
  }
  const double kEpsilon = 0.001;
  if (s + kEpsilon < accumulated_s_.front()) {
  ...
  if (iter == accumulated_s_.end()) {
    EDEBUG << "Reach the end of lane.";
    return 0.0;
  }
  int index = std::distance(accumulated_s_.begin(), iter);
  if (index == 0) {
    EDEBUG << "Reach the beginning of lane";
    return 0.0;
  } else {
...
bool LaneInfo::IsOnLane(const EDrive::common::math::Box2d &box) const {
...
PointENU LaneInfo::GetSmoothPoint(double s) const {
  ...
  if (delta_s < EDrive::common::math::kMathEpsilon) {
...
void LaneInfo::CreateKDTree() {
  EDrive::common::math::AABoxKDTreeParams params;
  ...
  for (size_t id = 0; id < segments_.size(); ++id) {
    ...
    segment_box_list_.emplace_back(
        EDrive::common::math::AABox2d(segment.start(), segment.end()), this,
        &segment, id);
  }
...
void StopSignInfo::UpdateOverlaps(const HDMapImpl &map_instance) {
  ...
  if (overlap_junction_ids_.size() <= 0) {
    EWARN << "stop sign " << id().id() << "has no overlap with any junction.";
  }
}
